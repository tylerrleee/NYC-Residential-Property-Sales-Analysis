...
setwd("/Users/tienle/Downloads/nycdata")
...
# head(queens) 4
bronx <- read.csv("rollingsales_bronx.csv") 
# head(bronx) 2
brooklyn <- read.csv("rollingsales_brooklyn.csv") 
# head(brooklyn) 3
statenisland <- read.csv("rollingsales_statenisland.csv") 
# head(statenisland) 5
#cpi <- read.csv("/Users/tienle/Downloads/nycdata/cpiannual.csv")
pluto <- st_read("MapPLUTO.shp") 
...
waffle_byresidential <- ggplot(residential_borough_counts, 
...
heat_map_price <- ggplot(heat_avg, aes(x = month_year, y = borough, fill = avg_price)) +
...
```{r Property Sales in NYC}
  property_sales <- ggplot() +
  # a) USA basemap
  ...
  labs(
    title ="Property Sales in NYC" 
  )
```
## 5.4 Property Sales Density Map

```{r Property Sale Density in NYC}
library(RColorBrewer)
property_density <-  ggplot() +
  #Base map of USA
  ...
  theme(
    ...
    labs(title = "Property Sale Density in NYC")
```

```{r Property Sales in each Borough}

# Clean the sale_price column if not done already
sales_geo <- sales_geo %>%
  mutate(sale_price = as.numeric(gsub(",", "", sale_price)))

# Define a function to create plots by borough
plot_borough_avg_price <- function(borough_name) {
  borough_data <- sales_geo %>%
    filter(borough == borough_name) %>%
    group_by(neighborhood) %>%
    summarize(avg_price = mean(sale_price, na.rm = TRUE)) %>%
    mutate(avg_price = avg_price / 1e5)

property_sale_borough <- ggplot(data = borough_data, aes(x = avg_price, y = reorder(neighborhood, avg_price), fill = neighborhood)) +
    geom_bar(stat = "identity") +
    labs(
      x = "Average Sale Price (in Thousands)",
      y = "Neighborhood",
      title = paste("Property Sales in", borough_name)
    ) +
    theme_minimal() +
...
plot_borough_avg_price("Manhattan")
plot_borough_avg_price("Queens")
plot_borough_avg_price("Brooklyn")
plot_borough_avg_price("Bronx")
plot_borough_avg_price("Staten Island")
```
```{r Price per Square Foot by Building Type}
# Normalize pricing across property sizes.
sales_geo <- sales_geo %>%
  mutate(price_per_sqft = sale_price / as.numeric(gross_sqft)) %>%
  filter(price_per_sqft < 5000)  # outlier trimming

sqr_ft_price <- ggplot(sales_geo, aes(x = building_class, y = price_per_sqft)) +
  geom_boxplot() +
  coord_flip() +
  labs(title = "Price per Square Foot by Building Type", x = "", y = "Price/Sqft ($)")
```

```{r Monthly Transaction Volume by Borough}
# Track market activity across months and boroughs.
sales_time <- all_sales %>%
  filter(sale_price > 0) %>%
  mutate(month = floor_date(date_operand, "month")) %>%
  group_by(month, borough) %>%
  summarise(sales = n())

volume_borough <- ggplot(sales_time, aes(x = month, y = sales, color = borough)) +
  geom_line(size = 1.1) +
  labs(title = "Monthly Transaction Volume by Borough", y = "Transactions", x = "Month")
```
...
ui <- fluidPage(
  titlePanel("NYC Property Sales Dashboard (2024-2025)"),
  sidebarLayout(
    sidebarPanel(
      class = "sidebar",
      width = 2,
      tabsetPanel(
        tabPanel("📊 Transaction Volume", value = "volume"),
        tabPanel("💲 Price/Sqft by Building Type", value = "price_sqft"),
        tabPanel("🗺️ Sales Bubble Map", value = "bubble_map"),
        tabPanel("🌡️ Sale Density Map", value = "density_map"),
        tabPanel("🔥 Price Heatmap", value = "heatmap"),
        tabPanel("🏘️ Residential Composition", value = "waffle"),
        tabPanel("📍 Neighborhood Avg Prices", 
                 value = "borough_avg",
                 selectInput("borough", "Select a Borough:",
                             choices = unique(sales_geo$borough),
                             selected = "Manhattan")
        ),
        id = "tabs"
      )
...
server <- function(input, output) {
  ...
  output$plot_ui <- renderUI({
      switch(input$tabs,
             "volume" = plotOutput("volume_plot"),
             "price_sqft" = plotOutput("price_sqft_plot"),
             "bubble_map" = plotOutput("bubble_map"),
             "density_map" = plotOutput("density_map"),
             "heatmap" = plotOutput("heatmap"),
             "waffle" = plotOutput("waffle_plot"),
             "borough_avg" = plotOutput("borough_bar")
      )
    })  
  # Transaction Volume
  ...
  output$price_sqft_plot <- renderPlot({
    sqr_ft_price
  })
  ...
  output$waffle_plot <- renderPlot({
    waffle_byresidential
  })

  # Borough-specific Neighborhood Avg Bar Chart
  output$borough_bar <- renderPlot({
    borough_data <- sales_geo %>%
    ...
    ggplot(borough_data, aes(x = avg_price, y = reorder(neighborhood, avg_price), fill = neighborhood)) +
      geom_bar(stat = "identity") +
      labs(x = "Avg Sale Price (in Thousands)",
           y = "Neighborhood",
           title = paste("Property Sales in", input$borough)) +
      theme_minimal() +
      theme(legend.position = "none")
  })
...
```{r, include=FALSE}
rsconnect::setAccountInfo(name='tylerrle', token='E311FA08D078AF1DCCE5D0ECA1FA2737', secret='TQRMGnFytglm0ZRVdBfFMoSEbhUzTLpPYAnva0Qq')
library(rsconnect)
rsconnect::deployApp(appDir = "NYCproperty.Rmd")
```
